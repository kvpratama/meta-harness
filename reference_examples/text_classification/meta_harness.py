...
def run_cmd(cmd, timeout=7200, cwd=None, verbose=False):
...
def run_benchmark(args, verbose=False):
    return run_cmd(
...
def propose(task_prompt, iteration, backend, model, effort, timeout=2400):
...
def validate_candidates(candidates, verbose=False):
...
def run_evolve(args):
    ...
    if not args.skip_baseline:
        ...
        for bl in baselines:
            ...
            t0 = time.time()
            result = run_benchmark(["--memory", bl], verbose=verbose)
        ...
        run_benchmark(["--frontier", "--model", model_short], verbose=verbose)
    ...
    for i in range(args.iterations):
        ...
        valid_candidates = validate_candidates(candidates, verbose=verbose)
        ...
        for ci, c in enumerate(valid_candidates):
            ...
            t0 = time.time()
            result = run_benchmark(["--memory", name], verbose=verbose)
            elapsed = time.time() - t0
            if result.returncode != 0:
    ...
    for name in sorted(test_systems):
        ...
        result = run_benchmark(["--memory", name, "--test"], verbose=verbose)
        if result.returncode != 0:
            print(f"    {_red('FAIL')} {name} test eval failed")

    run_benchmark(["--frontier", "--test", "--model", model_short], verbose=verbose)

    result = run_benchmark(["--results"], verbose=verbose)
...
def main():
    ...
    _prop_cfg = _cfg.get("proposer", {}) or {}
    _prop_backend = _prop_cfg.get("backend", "claude")
    parser.add_argument(
        "--proposer",
        choices=["claude", "opencode"],
        default=_prop_backend,
        help=f"Proposer backend (default: {_prop_backend})",
    )
    parser.add_argument(
        "--propose-timeout",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Forward subprocess stdout to terminal",
    )
    args = parser.parse_args()

    _bcfg = (_prop_cfg.get(args.proposer) or {})
    args.proposer_model = _bcfg.get("model") or ("opus" if args.proposer == "claude" else None)
    args.proposer_effort = _bcfg.get("effort")
    if args.proposer == "opencode" and not args.proposer_model:
        print("ERROR: proposer.opencode.model must be set in config.yaml (provider/model)")
        sys.exit(1)
